...
const hasActiveFilter = computed(() => !!filterCustomerId.value || !!filterDateFrom.value || !!filterDateTo.value)

const filters = computed<SaleOrderListFilters | undefined>(() => {
  const c = filterCustomerId.value || undefined
  const from = filterDateFrom.value || undefined
  const to = filterDateTo.value || undefined
  if (!c && !from && !to) return undefined
  return { customer_id: c, date_from: from, date_to: to }
})

function getCustomerName(row: SaleOrderWithCustomer): string {
...
async function openFilterPopup() {
  ...
  if (customers.value.length === 0) {
    try {
      ...
      showToast({ type: 'fail', message: '加载客户列表失败' })
    }
  }
}
  ...
  <div class="sale-order-list">
    ...
    <van-popup v-model:show="filterVisible" position="bottom" round class="filter-popup">
      ...
      <van-cell-group inset>
        <van-cell title="客户">
          <template #value>
            <select v-model="filterCustomerId" class="filter-select">
