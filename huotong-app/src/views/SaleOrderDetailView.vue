...
import type { SaleOrder } from '../composables/useSaleOrders'
import type { SaleOrderItemWithProduct } from '../composables/useSaleOrders'
import { requestRetryOnNetworkError } from '../lib/networkRetry'

const route = useRoute()
const orderId = computed(() => route.params.id as string)

const { getById, getItemsWithProduct, confirm, parseConfirmError, loading: confirming } = useSaleOrders()
const { getById: getCustomerById } = useCustomers()

const order = ref<SaleOrder | null>(null)
const customerName = ref('')
const items = ref<SaleOrderItemWithProduct[]>([])
...
async function loadOrder() {
  ...
  try {
    ...
    order.value = o
    if (o) {
      const cust = await getCustomerById(o.customer_id)
...
async function handleConfirm() {
  ...
  try {
    ...
    showToast({ type: 'success', message: '出货单已确认' })
    await loadOrder()
  } catch (e) {
    const shouldRetry = await requestRetryOnNetworkError(e, {
      title: '网络异常',
      message: '确认出货失败，请检查网络后重试',
...
<template>
  <div class="sale-order-detail">
    ...
    <template v-else-if="!order">
      <div class="empty">出货单不存在</div>
    </template>
    <template v-else>
      <van-cell-group inset>
        <van-cell title="单号" :value="order.order_no" />
        <van-cell title="客户" :value="customerName" />
        <van-cell title="状态">
          <template #value>
