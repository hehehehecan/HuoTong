...
| FR44 | Epic 8 | Android APK 安装与主入口 |
| FR45 | Epic 9 | 会话恢复与应用生命周期 |
| FR46 | Epic 9 | Android 返回键导航体验 |
| FR47 | Epic 10 | 网络异常提示与重试 |
| FR48 | Epic 11 | APK 覆盖安装升级与版本管理 |
| FR49 | Epic 12 | 家庭内部安装指引、账号分发、反馈闭环 |
| FR50 | Epic 10 | Android 首版降级增强能力但保留核心业务 |
...
建立 Android 客户端基础工程，让家人可以通过 APK 安装并从桌面图标进入系统。
**FRs covered:** FR44

### Epic 9: Android 运行时适配
让现有业务壳在 Android 中具备可接受的会话恢复、返回键和前后台切换体验。
**FRs covered:** FR45, FR46

### Epic 10: Android 首版稳定性与降级策略
围绕网络提示、手动重试和功能降级，确保首版以稳定可用为先。
**FRs covered:** FR47, FR50

### Epic 11: Android 版本升级与分发管理
建立可持续维护的 APK 版本、签名和覆盖安装升级机制。
**FRs covered:** FR48

### Epic 12: 家庭内测交付与上线准备
补齐账号分发、安装指引、真机验证和反馈闭环，支持家人真正开始使用。
**FRs covered:** FR49
...
**Given** 现有 Web 项目已可正常构建
**When** 开发者接入 Capacitor 并生成 Android 工程
**Then** 项目可以输出可安装的 Android 调试包
**And** Android 工程能正确加载本地前端资源，而不是依赖线上网页入口

**Given** Android 客户端首次启动
**When** 用户从桌面点击应用图标
**Then** 应用正常打开 HuoTong 登录页或首页
**And** 不经过浏览器地址栏和网页托管入口
...
**Given** Android 工程已创建
**When** 配置应用名、包名、图标、启动图、`versionName`、`versionCode`
**Then** 调试包与发布包都具备明确的应用身份信息

**Given** 开发者准备发布首版 APK
**When** 使用正式 keystore 签名
**Then** 生成的 APK 可作为后续所有版本覆盖安装升级的基础
**And** keystore 保管方式被记录下来
...
**When** App 退到后台后再次恢复
**Then** 系统优先恢复现有会话
**And** 若会话失效则给出明确提示并引导重新登录

**Given** 用户从后台恢复到关键列表页
**When** 页面重新激活
**Then** 系统刷新必要数据或提供明显的手动刷新入口
**And** 不依赖 Realtime 才能看到最新数据
...
**Given** 用户正在二级页面
**When** 按下 Android 返回键
**Then** 页面回到上一级，而不是直接退出应用

**Given** 页面上有打开的弹窗、抽屉或选择器
**When** 用户按下返回键
**Then** 优先关闭当前弹层

**Given** 用户位于首页
**When** 按下返回键
**Then** 系统执行退出确认或符合预期的首页退出行为
...
**Given** Supabase 请求失败或超时
**When** 页面无法完成关键操作（登录、列表加载、保存、确认）
**Then** 显示清晰中文错误提示
**And** 提供显式的重试入口
...
**Given** Android 首版发布配置
**When** 应用首次交付给家人
**Then** Realtime、拍照识别、浏览器式导出下载等高风险能力默认关闭或隐藏
**And** 登录、商品、客户、供应商、进货、出货、应收应付、库存等核心流程保持可用

**Given** 用户进入不再推荐的历史 Web/桌面特化功能
**When** 该功能不适合 Android 首版
**Then** 系统隐藏入口或给出明确说明，而不是暴露半完成体验
...
**Given** 开发者生成新版 APK
**When** 版本号递增并进行签名
**Then** 新 APK 可以覆盖安装旧版本
**And** 用户原有登录态和使用数据不会因升级而异常丢失

**Given** 用户在 App 内查看关于页或设置页
**When** 页面展示版本信息
**Then** 能看到当前版本号，便于反馈问题和确认是否更新
...
**Given** 首版 APK 已准备完成
**When** 开发者整理交付材料
**Then** 至少包含安装步骤、未知来源授权说明、登录方式、忘记密码处理和常见问题

**Given** 家人收到安装包和账号信息
**When** 按说明操作
**Then** 可以独立完成安装并成功登录系统
...
**Given** Android 首版候选包已可安装
**When** 在至少一台真实家庭使用手机上验证核心流程
**Then** 完成登录、商品/客户/供应商、进货、出货、应收应付、库存、前后台切换等回归检查

**Given** 家人在试用过程中遇到问题
**When** 反馈问题给开发者
**Then** 至少能同时提供版本号、机型、网络环境和问题复现步骤
**And** 开发者可以据此判断是否需要重新发版
